  ...
  if (BFD_IF_DEBUG_NET)
    {
      ...
      zlog_debug ("RECV packet from %s:%u to %s:%u on %s",
		  sockunion2str (rem, lbuf, SU_ADDRSTRLEN), rport,
		  sockunion2str (loc, rbuf, SU_ADDRSTRLEN), lport, ifindex2ifname (ifindex));
    }
...
#define IPV6_NOTTL_CHECK
  ...
  if (ifp == NULL)
    {
      if (BFD_IF_DEBUG_NET){
	    ...
	    zlog_debug ("%s: cannot find interface for packet from %s port %d",
		    __func__, sockunion2str (rem, buf, SU_ADDRSTRLEN), rport);
	}
      return -1;
    }
  ...
  /* "If the Your Discriminator field is zero, the session MUST be
    ...
    {
      struct prefix *lp = sockunion2hostprefix (loc, NULL);
      struct prefix *rp = sockunion2hostprefix (rem, NULL);
	  ...
	  if (BFD_IF_DEBUG_NET){
	    ...
	    zlog_debug
	      ("%s: Unable to demultiplex session src=%s:%d/dst=%s:%d on intf=%s.",
	       __func__, 
	       sockunion2str (rem, lbuf, SU_ADDRSTRLEN), ntohs (rem->sin.sin_port),
	       sockunion2str (loc, rbuf, SU_ADDRSTRLEN), ntohs (loc->sin.sin_port),
