...
{
  struct prefix remote, local;

  sockunion2hostprefix (neighp->su_remote, &remote);
  sockunion2hostprefix (neighp->su_local, &local);
  return bfd_find_neigh (&remote, &local, 0);			 
}
...
bfd_neightbl_raddr_adddel (int cmd, struct bfd_neigh *neighp)
{
  ...
  struct bfd_addrtreehdr *hdrp;
  struct neighstruct *ns;
  struct prefix raddr, laddr;

  sockunion2hostprefix (neighp->su_remote, &raddr);
  sockunion2hostprefix (neighp->su_local, &laddr);
...
bfd_neigh_trytostart (struct bfd_neigh *neighp)
{
  if (bfd_neigh_check (neighp))
    {
      if (BFD_IF_DEBUG_ZEBRA)
		BFD_LOG_DEBUG_NEIGH_ARG ("%s: unable to start bfd session this time", __func__) 
	  return BFD_ERR;
    }
  if (BFD_IF_DEBUG_ZEBRA)
