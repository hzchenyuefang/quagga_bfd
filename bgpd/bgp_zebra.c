...
bgp_bfd_neigh_add(struct peer *peer)
{
  ...
  if(peer->status == Established)
  {
	...
	sockunion2hostprefix(&peer->su, &remote);
	sockunion2hostprefix(peer->bfd_su_local, &local);
    ...
    if (BGP_DEBUG(zebra, ZEBRA)) {
      ...
      zlog_debug("Zebra send: bfd cneigh add "
		 ...
		 sockunion2str(&peer->su, lbuf, SU_ADDRSTRLEN), 
		 sockunion2str(peer->bfd_su_local, rbuf, SU_ADDRSTRLEN),
		 peer->bfd_ifindex, peer->bfd_flags);
    }

    if(sockunion_family(&peer->su) == AF_INET) 
      zapi_ipv4_bfd_cneigh_add(zclient,
	  (struct prefix_ipv4*)&remote,
	  (struct prefix_ipv4*)&local,
	  peer->bfd_ifindex,
	  peer->bfd_flags);    
#ifdef HAVE_IPV6
    else 
      zapi_ipv6_bfd_cneigh_add(zclient,
	  (struct prefix_ipv6*)&remote,
	  (struct prefix_ipv6*)&local,
	  peer->bfd_ifindex,
	  peer->bfd_flags);
#endif /* HAVE_IPV6 */
